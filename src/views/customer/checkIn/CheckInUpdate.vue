    ...
    <div>
        <el-card class="box-card">
            <div class="header">
                ...
                <h4 style="text-align:center;color: gray;margin-bottom: 20px">入住登记修改</h4>
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        methods: {
            //根据主键加载数据
            getData(){
                axios.get('/customer/get/' + this.ruleForm.id).then(res=>{
                    this.ruleForm = res.data;
                    this.oldBed = this.ruleForm.bedId;
                })
            },
            getBeds() {
            ...
            check_In(id){
                axios.post("/bed/checkIn/" + id).then(res => {
            ...
            submitForm() {
                if(this.oldBed !== this.ruleForm.bedId){
                    this.withdraw(this.oldBed);
                    this.check_In(this.ruleForm.bedId);
                }
                axios.post('/customer/update', this.ruleForm).then(res => {
