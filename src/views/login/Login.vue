    ...
    <div class="login-wrap">
        <el-form label-position="left" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="0px"
            ...
            <el-form-item prop="username">
                <el-input class="input" prefix-icon="el-icon-user-solid" type="text" v-model="ruleForm.username"
                          auto-complete="off" placeholder="用户名"></el-input>
            </el-form-item>
            <el-form-item prop="password">
                <el-input class="input" prefix-icon="el-icon-lock" type="password" v-model="ruleForm.password"
                          auto-complete="off" placeholder="密码"></el-input>
            </el-form-item>
            <el-row>
                <el-col :span="12">
                    <el-form-item prop="verifycode">
                        <el-input prefix-icon="el-icon-s-check" type="text" class="identity"
                                  v-model="ruleForm.verifycode" auto-complete="off" placeholder="图形验证码"
                                  @keyup.enter.native="submitForm('ruleForm')"></el-input>
                    </el-form-item>
                </el-col>
                <el-col :span="12" class="code-box">
                    <span @click="refreshCode" class="codeimg"><v-identity :identifyCode="identifyCode"/></span>
                </el-col>
            </el-row>
        ...
        methods: {
            remember() {//记住密码
                if (this.rememberpwd === true) {
                    this.setCookie(this.rememberpwd, this.ruleForm.username, this.ruleForm.password, 7)
                } else {
                    this.clearCookie();
                }
            },
                    ...
                    if (valid) {
                        ...
                        axios.post("user/login/", this.ruleForm).then(res => {
                            switch (res.data.status) {
                                ...
                                case 2:
                                    this.logining = true;
                                    // var user = JSON.stringify(res.data.obj);
                        ...
                        this.$message.error('请输入用户名密码及验证码！');
                        this.logining = false;
                        return false
                    }
                })
            ...
            change(val) {
                let rem = val === 'true';
                this.rememberpwd = rem;
            },
            //设置cookie
            setCookie(c_rememberpwd, c_name, c_pwd, exdays) {
                var exdate = new Date(); //获取时间
                exdate.setTime(exdate.getTime() + 24 * 60 * 60 * 1000 * exdays); //保存的天数
                //字符串拼接cookie
                window.document.cookie = "rememberpwd" + "=" + c_rememberpwd + ";path=/;expires";
            ...
            getCookie() {
                if (document.cookie.length > 0) {
                    ...
                    for (var i = 0; i < arr.length; i++) {
                        ...
                        //判断查找相对应的值
                        if (arr2[0] === 'rememberpwd') {
                            this.change(arr2[1]); //保存到保存数据的地方
